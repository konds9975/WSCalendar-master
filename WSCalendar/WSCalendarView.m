...
{
    __weak IBOutlet UILabel *lastLine;
    NSMutableArray *dayInMonth;
    NSMutableArray *completeDateArray;
    NSArray *lblArray,*weekLblArray;
    BOOL isLoop;
    BOOL is;
    NSDate *selectedDate;
}
...
-(void)initializeMonthYear
{
    ...
    weekLblArray = [[NSArray alloc] initWithObjects:lbl1,lbl2,lbl3,lbl4,lbl5,lbl6,lbl7,nil];
    isLoop=YES;
    is=YES;
...
-(void)setMonthLabels:(NSDate *)date
{
    ...
    for (int i=0; i<dayInMonth.count; i++) {
        ...
        if (i < 7)
        {
            UILabel *lbl1=[weekLblArray objectAtIndex:i];
            NSDateFormatter *weakFormattar=[[NSDateFormatter alloc] init];
            [weakFormattar setDateFormat:@"EEE"];
            lbl1.text=[weakFormattar stringFromDate:[dayInMonth objectAtIndex:i]];
...
-(void)setYear:(NSDate *)date
{
    ...
    [monthFormatter1 setDateFormat:@"MMM YYYY"];
    NSDateFormatter *monthFormatter=[[NSDateFormatter alloc] init];
    [monthFormatter setDateFormat:@"MMMM"];
    [btnBarDate setTitle:[monthFormatter stringFromDate:date]];
    NSDateComponents *dateComponent1=[[NSDateComponents alloc] init];
    
    [dateComponent1 setMonth:-1];
    NSDate *datein1 = [[NSCalendar currentCalendar] dateByAddingComponents:dateComponent1 toDate:selectedDate options:0];
    //[barBtnprvmonth setTitle:[monthFormatter stringFromDate:datein1]];
    [barBtnprvmonth setTitle:[NSString stringWithFormat:@"< %@",[monthFormatter1 stringFromDate:datein1]]];
    
    NSDateComponents *dateComponent12=[[NSDateComponents alloc] init];
    
    [dateComponent12 setMonth:1];
    NSDate *datein12 = [[NSCalendar currentCalendar] dateByAddingComponents:dateComponent12 toDate:selectedDate options:0];
    [barBtnnextMonth setTitle:[NSString stringWithFormat:@"%@ >",[monthFormatter1 stringFromDate:datein12]]];
}
